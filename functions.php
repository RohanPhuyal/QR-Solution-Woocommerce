	...
	function kwp_yape_peru_payment_popup()
	{
		...
		<div class="kwp-qr-data-hidden" style="display: none;">
			<?php foreach ($qr_options as $index => $qr_option): ?>
				<div class="kwp-qr-data-item" data-index="<?php echo esc_attr($index); ?>"
					...
					<?php if (count($qr_options) > 1): ?>
						<div class="kwp-popup-qr-selector">
							...
							<div class="kwp-popup-qr-options">
								<?php foreach ($qr_options as $index => $qr_option):
									if (empty($qr_option['qr_image']))
										continue;
									$is_first = ($index === array_key_first($qr_options));
									?>
									<div class="kwp-popup-option-item <?php echo $is_first ? 'active' : ''; ?>"
										data-index="<?php echo esc_attr($index); ?>">
										<?php if (!empty($qr_option['icon_image'])): ?>
											<img src="<?php echo esc_url($qr_option['icon_image']); ?>"
												alt="<?php echo esc_attr($qr_option['name']); ?>" />
										<?php endif; ?>
				...
				<div class="second-step">
					<form method="post" enctype="multipart/form-data" novalidate="" class="box has-advanced-upload">
						...
						<input type="hidden" name="selected_qr_option" class="selected-qr-option-input"
							value="<?php echo esc_attr($first_option['name']); ?>">
					</form>
...
function kwp_display_remaining_admin($order_id)
{
	...
	if (!$order || $order->get_payment_method() !== 'wocommerce_yape_peru')
		return;
...
{
	if ($order->get_payment_method() !== 'wocommerce_yape_peru') {
		echo '';
		return;
	}
	$remaining = $order->get_meta('_remaining_to_pay');
